...
const ScrambledTitle: React.FC<ScrambledTitleProps> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    const next = () => {
      if (cancelled || !scramblerRef.current) {
        return
      }

      const currentIndex = counter
      scramblerRef.current.setText(phrases[currentIndex]).then(() => {
        if (cancelled) {
          return
        }
        ...
        timeoutRef.current = window.setTimeout(next, 1200)
      })
    }
...
const RainingLetters: React.FC<RainingLettersProps> = ({
  ...
  useEffect(() => {
    ...
    const flickerInterval = window.setInterval(updateActiveIndices, 50)
        ...
        prevChars.map((char) => ({
          ...
          ...(char.y >= 100 && {
            ...
            char: "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+-=[]{}|;:,.<>?"[
              Math.floor(
                Math.random() * "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789!@#$%^&*()_+-=[]{}|;:,.<>?".length
              )
            ],
          }),
        }))
  ...
  return (
    <div className={`relative w-full h-screen bg-transparent overflow-hidden ${className}`}>
        <div className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 transition-opacity duration-1000 ${showTitle ? 'opacity-100' : 'opacity-0'}`}>
